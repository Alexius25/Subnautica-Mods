...
public static class BedPrefabLoader
{
    ...
    public class BedConfig
    {
        ...
        public string icon;                     // Icon filename (optional, will use base bed icon if not provided)
    ...
    public static void RegisterCustomBeds(string imageFolder)
    {
        ...
        foreach (var configPath in configFiles)
        {
            ...
            if (config.icon != null)
            {
                // Load icon sprite from file
                string iconPath = Path.Combine(folder, config.icon);
                if (!File.Exists(iconPath))
                {
                    Debug.LogWarning($"[BedPrefabLoader] Icon file not found: {iconPath}, using default icon.");
                    // Fallback to icon from the chosen bed type
                    prefabInfo.WithIcon(SpriteManager.Get(baseBedType));
                }
                else
                {
                    var iconTexture = LoadTextureFromFile(iconPath);
                    if (iconTexture != null)
                    {
                        var icon = Sprite.Create(iconTexture, new Rect(0, 0, iconTexture.width, iconTexture.height), new Vector2(0.5f, 0.5f));
                        prefabInfo.WithIcon(icon);
                        Debug.Log($"[BedPrefabLoader] Loaded custom icon for {bedName} from {iconPath}");
                    }
                    else
                    {
                        Debug.LogWarning($"[BedPrefabLoader] Failed to load icon texture from {iconPath}");
    ...
    /// <summary>
    /// Make a Texture2D readable by cloning it into a new Texture2D.
    /// </summary>
